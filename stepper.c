...
//    _delay_us(1);         // stepper driver and cpu plenty fast to skip the second interrupt
//    set_step_pins(0);
...
///      OCR1A = st.exec_segment->cycles_per_tick;
      ...
      set_step_timer_period(st.exec_segment->cycles_per_tick);
  ...
  while (segment_buffer_tail != segment_next_head) { // Check if we need to fill the buffer.
    ...
    */
    ...
    uint32_t cycles = ceil( (TICKS_PER_MICROSECOND*1000000*60)*inv_rate ); // (cycles/step)    
      ...
      else { prep_segment->cycles_per_tick = 0xfff0; } // Just set the slowest speed possible.
    #else 
