...
static void protocol_execute_line(char *line1) 
{      
  protocol_execute_realtime(); // Runtime command check point.
  if (sys.abort) { return; } // Bail to calling function upon system abort  

  #ifdef REPORT_ECHO_LINE_RECEIVED
    report_echo_line_received(line1);
  #endif
  ...
  if (line[0] == 0) {
    ...
    // Grbl '$' system command
    report_status_message(system_execute_line(line1));
    ...
    report_status_message(gc_execute_line(line1));
  }
}
        ...
        if ((sys.state == STATE_IDLE) || ((sys.state & (STATE_HOLD | STATE_MOTION_CANCEL)) && (sys.suspend & SUSPEND_ENABLE_READY))) {
          ...
          if (sys.suspend & SUSPEND_ENERGIZE) { 
            ...
            if (gc_state.modal.spindle != SPINDLE_DISABLE) { 
              ...
              _delay_ms(SAFETY_DOOR_SPINDLE_DELAY); // TODO: Blocking function call. Need a non-blocking one eventually.
            }
            if (gc_state.modal.coolant != COOLANT_DISABLE) { 
              coolant_set_state(gc_state.modal.coolant); 
              _delay_ms(SAFETY_DOOR_COOLANT_DELAY); // TODO: Blocking function call. Need a non-blocking one eventually.
            }
